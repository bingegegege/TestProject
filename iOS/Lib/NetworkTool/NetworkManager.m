       ...
       failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    //开始请求内容
    [_sessionManager GET:url_all parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
  ...
  NSString *url_all = [NSString stringWithFormat:@"%@%@",AppBase_url,url];
    //开始请求内容
    [_sessionManager POST:url_all parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        //如果需要填充进度内容，可以直接进行内容添加
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        successful(responseObject);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
                ...
                failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    NSString *url_all = [NSString stringWithFormat:@"%@%@",AppBase_url,url];
    [_sessionManager POST:url_all parameters:paramters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //对上传完文件的配置
        //获取当前时间（int 时间戳转换）
