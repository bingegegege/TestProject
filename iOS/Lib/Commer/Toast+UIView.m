...
// activity
static const CGFloat CSToastActivityWidth       = 120.0;//100.0  sff
...
- (void)makeToastActivity {
    ...
    [self makeToastActivity:CSToastActivityDefaultPosition withText:nil];
}

- (void)makeToastActivity:(id)position test:(NSString *)test;
{
    ...
    activityView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.5];
    ...
    label.frame = CGRectMake(activityView.bounds.size.width/2 - CSToastActivityWidth/2, activityIndicatorView.frame.origin.y + 30, CSToastActivityWidth, 40);
    label.text = test;
...
- (void)makeToastActivity:(id)position withText:(NSString *)text{
    ...
    UIView *activityView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth, CSToastActivityHeight)] autorelease];
    activityView.center = [self centerPointForPosition:position withToast:activityView];
    activityView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.7];
    activityView.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin);
    activityView.layer.cornerRadius = CSToastCornerRadius;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - 25);
